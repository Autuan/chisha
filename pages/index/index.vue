<template>
	<view class="conbox">
		<view class="container">
			...
			<view>
				<button class="btn btn-warn settValBox " style="text-align: center; margin-left: 40%;margin-top: 200upx;" @click="open" size="mini" type="warn">设置内容</button>
				<uni-popup ref="popup" type="dialog" class="settValBox">
					<uni-popup-dialog type="info" mode="input" placeholder="英文,分割"  :duration="2000" :before-close="true"
	...
	export default {
		...
		methods: {
			...
			confirm(done, value) {
				...
				this.val = value;
				// userVal
				uni.setStorageSync("userVal",value);
			...
			//发起抽奖
			playReward() {
				let index = Math.random() * this.list.length | 0;
				let duration = 4000;
				this.animation(index, duration)

				setTimeout(() => {
					uni.showModal({
						content: this.list[index].isNoPrize ? '抱歉，您未中奖' : '就吃这个了：' + this.list[index].name
					})
					this.btnDisabled = '';
			...
			setVal() {
				let userVal = uni.getStorageSync("userVal");
				if (userVal) {
					
				this.val = userVal;
				}
				
				let splitList = this.val.split(',');
				let inputList = [];
				...
				this.list = inputList;
				this.width = 360 / inputList.length
			},
		}
